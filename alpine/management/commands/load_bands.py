...
from . import bands
from alpine.models import Band
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        Band.objects.bulk_create([Band(name=b, genre="Punk") for b in bands.PUNK])
        Band.objects.bulk_create(
            [Band(name=b, genre="Post Punk") for b in bands.POST_PUNK]
        )
        Band.objects.bulk_create([Band(name=b, genre="Jazz") for b in bands.JAZZ])
        Band.objects.bulk_create([Band(name=b, genre="Hip Hop") for b in bands.HIP_HOP])
        Band.objects.bulk_create(
